...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        int bytes = recvfrom(sock, buffer, BUFFER_SIZE - 1, 0, (struct sockaddr*)&from_addr, &from_len);
        if (bytes <= 0) break;
        buffer[bytes] = '\0';
        ...
        if (strncmp(buffer, "MATCH", 5) == 0) {
            ...
            sscanf(buffer, "MATCH %d %s %s %s %d", &opp_id, your_choice, opp_choice, result, &points);
